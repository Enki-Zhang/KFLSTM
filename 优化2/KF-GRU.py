...
KFGRU = pd.read_csv('KFGRUPRE110003.csv')
...
plt.scatter(observed[1500:], KFGRU[1500:], label='Train set',marker='^',color='red')

plt.scatter(observed[:1000], KFGRU[:1000], label='Test set',marker='o',color='blue')

# plt.scatter(observed, predictedKFLSTM, label='KF-LSTM',marker='o',color='#fe8600')
plt.text(0.75, 0.28, fr'$R^2_{{\mathrm{{train}}}}={0.88:.2f}$',ha='center', va='center',
         transform=plt.gca().transAxes, fontdict={'family': 'serif', 'style':'oblique','size': 20})
plt.text(0.75, 0.18, fr'$R^2_{{\mathrm{{test}}}}={0.87:.2f}$',ha='center', va='center',
         transform=plt.gca().transAxes, fontdict={'family': 'serif', 'style':'oblique','size': 20})

lr_label = plt.text(11.42, -0.35,
                    f'(f)KF-GRU', ha='right', va='bottom',fontsize=20 )
# 设置LR标签背景颜色为灰色
