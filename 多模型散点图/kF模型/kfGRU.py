...
# 提取观测值和预测值列
observed = pd.read_csv('TRUE1000.csv')
predictedLSTM = pd.read_csv('LSTMPRE11000.csv')
predictedKFLSTM = pd.read_csv('KFLSTMPRE10002.csv')
...
# plt.scatter(observed, predictedLSTM, label='LSTM',marker='o',color='blue')
# plt.scatter(observed, predictedKFLSTM, label='KF-LSTM',marker='o',color='blue')
plt.scatter(observed, predictedKFGRU, label='KF-GRU', marker='o', color='blue')
plt.text(0.85, 0.18, fr'$R^2={0.87:.2f}$', ha='center', va='center', transform=plt.gca().transAxes,
         fontdict={'family': 'serif', 'style': 'oblique', 'size': 20})

# 添加标签
lr_label = plt.text(8.5, 0.08, f'(e)KF-GRU', fontsize=16, )
# 设置LR标签背景颜色为灰色
