...
# 绘制散点图
y_train = pd.read_csv('MLPTrainLabel2.csv')
y_train_pred = pd.read_csv('MLPTrainPre2.csv')
y_test = pd.read_csv('MLPLabel2.csv')
y_test_pred = pd.read_csv('MLPPre2.csv')
...
plt.text(0.82, 0.28, fr'$R^2_{{\mathrm{{train}}}}={0.28:.2f}$',ha='center', va='center',
         transform=plt.gca().transAxes, fontdict={'family': 'serif', 'style':'oblique','size': 20})
plt.text(0.82, 0.18, fr'$R^2_{{\mathrm{{test}}}}={0.30:.2f}$',ha='center', va='center',
         transform=plt.gca().transAxes, fontdict={'family': 'serif', 'style':'oblique','size': 20})
