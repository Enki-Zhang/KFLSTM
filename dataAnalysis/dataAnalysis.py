...
newData = pd.read_csv(Path + "原始数据平滑训练和验证集不平滑测试集.csv", usecols=[0, 1, 2, 3, 4])
WData = np.array(data)
...
for i, name in enumerate(data.columns):  # Index(['time', 'tem', 'PH', 'oxy', 'elec', 'tur'], dtype='object')
    ...
    # plt.plot(WData[8345:8954, i], c=color[0], label=name, alpha=0.3)  # 原始数据4
    # plt.plot(NData[8345:8954, i], c=color[2], label=name + ' smooth')
    if name == 'tem':
    ...
    plt.legend()
    # plt.savefig("./pic/" + "dataAndSmooth" + name + ".png")
    plt.show()
    print(i, name)
